...
class CameraManager {
     ...
     */
    func captureSetup(in cameraView: UIView, withPosition cameraPosition: AVCaptureDevice.Position? = .back) {
    ...
    func getImage(croppWith rect: CGRect? = nil, completionHandler: @escaping (UIImage?, Error?) -> Void){
        ...
        if let videoConnection = videoDataOutput?.connection(with: AVMediaType.video) {
